      ...
      <div class="activity-film">
        ...
        <div class="box" v-for="(item,index) in activity.prizeList" :key="index">
            ...
            <div class="film-bottom">
              ...
              <span class="btn" v-if="activity.drawType==='2'" :class="btnStatus.disabled&&(!!activity.remainDrawNum ||!userInfo.userId)?'':'disabled'" 
                @click="isLukyClick(item.prizeId,activity.remainDrawNum)">
                  <i v-if="!btnStatus.disabled">{{btnStatus.text}}</i>
                  <i v-else>{{activity.remainDrawNum||!userInfo.userId?'立即抢票':'机会用完了'}}</i>
                </span>

                <span class="btn" v-if="activity.drawType==='1'"  :class="btnStatus.disabled&&(!!item.remainDrawNum ||!userInfo.userId)?'':'disabled'" 
                @click="isLukyClick(item.prizeId,item.remainDrawNum)">
                  <i v-if="!btnStatus.disabled">{{btnStatus.text}}</i>
        ...
        <div class="change">
          ...
          <div v-else @click='isLogin' class="blue"><em class="block">您还未登陆，点击登陆</em></div>
        </div>

      </div>

      <div class="more-film-img frbm">
          <img src="http://p0bd8izdn.bkt.clouddn.com/ccb/wap/images/more_film_img1.png"  @click="$util.goToWap">
          <i class="poker-img"  @click.stop="moreCoupoons"></i>
        </div>
...
.body {
    ...
    .more-film-img {
      ...
      img{width: 170px;}
      .poker-img{width: 170px;height: 170px;background: url('http://p0bd8izdn.bkt.clouddn.com/ccb/wap/images/more_film_img2.gif')no-repeat;background-size: 100% 100%;}
    }
    .hot-film {
        background: #ffffff;
        padding: 0px 20px;
